...
> **On scrolling and "seamless wrap":** the GameTank has **no** hardware scroll
> register and the blitter cannot read VRAM as a source, so there is no free
> screen-shift. The GRAM-canvas window above IS the hardware-optimal scroll - a
> larger-than-screen canvas composed once, then one windowed blit per frame. For
> a *looping* world, compose a 256×256 torus canvas and window it with wrapped
> coordinates (`gt.track_view(camx & 255, camy & 255)`); the blitter's edge-clip
> handles the seam. (The blitter's `WRAP` bit is an edge-*clip*, not a
> wrap-around - it suppresses off-screen pixels; the SDK sets it on every blit.)

### Use hardware sprite flip - don't bake mirrored art

`spr(n, x, y, w, h, flipx, flipy)` - the last two flags are **free hardware
mirroring**. The blitter reads the source cell reversed (it sets the WIDTH/HEIGHT
bit-7 XDIR/YDIR flags); there is **zero** extra CPU or GRAM cost vs an unflipped
blit.

So a character that faces both ways needs **one** set of cells, not two:

```lua
-- WASTEFUL: two mirrored copies of the same art on the sheet
if facing == 1 then spr(walk_r, x, y) else spr(walk_l, x, y) end

-- FREE: one set of cells, hardware mirror for the other facing
spr(walk, x, y, 1, 1, facing == -1, false)   -- flipx when facing left
```

This halves the sheet cells you spend on any left/right (or up/down) symmetric
art - GRAM and, on a FLASH2M cart, ROM you get back for free. Flip is exact and
verified; it costs the same as a normal blit. Recoloring is baked into the sheet
(there is no `pal()` - framebuffer bytes are colors), so a *recolored mirror*
still needs its own cell - but a *plain* mirror never should.
