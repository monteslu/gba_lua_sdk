/* generated by gbalua from main.lua - edit the .lua, not this file */
#include "gba_api.h"

static void lcl__init(void);
static void lcl_fire(void);
static void lcl__update(void);
static void lcl__draw(void);

int lcl_px = 108;
int lcl_py = 130;
int lcl_pcool = 0;
int lcl_bx[8];
int lcl_by[8];
unsigned char lcl_ba[8];
int lcl_enx[12];
int lcl_eny[12];
unsigned char lcl_eal[12];
int lcl_edir = 1;
int lcl_edx = 0;
int lcl_alive = 12;
int lcl_score = 0;
int lcl_lives = 3;
int lcl_state = 0;
int lcl_scrolly = 0;
int lcl_fadet = 0;
int lcl_endt = 0;

static void lcl__init(void)
{
    gba_map_show(0);
    gba_music(0, 1);
    { unsigned char lcl_i = 1; unsigned char L_lim0 = 12;
        for (; lcl_i <= L_lim0; ++lcl_i) {
            { int lcl_col = (((lcl_i - 1)) % (6));
                { int lcl_row = (((lcl_i - 1)) / (6));
                    lcl_enx[lcl_i - 1] = (24 + (((lcl_col) << 5)));
                    lcl_eny[lcl_i - 1] = (20 + (((lcl_row) << 4) + ((lcl_row) << 3)));
                    (lcl_eal - 1)[lcl_i] = 1;
                }
            }
        }
    }
    { unsigned char lcl_i = 1; unsigned char L_lim1 = 8;
        for (; lcl_i <= L_lim1; ++lcl_i) {
            (lcl_ba - 1)[lcl_i] = 0;
        }
    }
}

static void lcl_fire(void)
{
    { unsigned char lcl_i = 1; unsigned char L_lim2 = 8;
        for (; lcl_i <= L_lim2; ++lcl_i) {
            if (((unsigned char)(lcl_ba - 1)[lcl_i] == (unsigned char)0)) {
                lcl_bx[lcl_i - 1] = (lcl_px + 4);
                lcl_by[lcl_i - 1] = lcl_py;
                (lcl_ba - 1)[lcl_i] = 1;
                gba_sfx(0, -1);
                return;
            }
        }
    }
}

static void lcl__update(void)
{
    if ((lcl_state != 0)) {
        if ((lcl_endt < 30)) {
            lcl_endt = (lcl_endt + 1);
        }
        gba_fade((long)(((long long)((long)((((long long)(((long)lcl_endt << 16))) << 16) / (1966080L))) * (45875L /* 0.7 */)) >> 16), 0);
        if (gba_btnp(4, 0)) {
            gba_run();
        }
        return;
    }
    if ((lcl_fadet < 30)) {
        lcl_fadet = (lcl_fadet + 1);
        gba_fade((65536L - (long)((((long long)(((long)lcl_fadet << 16))) << 16) / (1966080L))), 0);
    }
    lcl_scrolly = (lcl_scrolly + 1);
    gba_camera(0, ((-lcl_scrolly) >> 1));
    if (gba_btn(0, 0)) {
        lcl_px = (lcl_px - 3);
    }
    if (gba_btn(1, 0)) {
        lcl_px = (lcl_px + 3);
    }
    if ((lcl_px < 4)) {
        lcl_px = 4;
    }
    if ((lcl_px > 220)) {
        lcl_px = 220;
    }
    if ((lcl_pcool > 0)) {
        lcl_pcool = (lcl_pcool - 1);
    }
    if ((gba_btn(4, 0) && (lcl_pcool == 0))) {
        lcl_fire();
        lcl_pcool = 8;
    }
    { unsigned char lcl_i = 1; unsigned char L_lim3 = 8;
        for (; lcl_i <= L_lim3; ++lcl_i) {
            if (((unsigned char)(lcl_ba - 1)[lcl_i] != (unsigned char)0)) {
                lcl_by[lcl_i - 1] = ((lcl_by - 1)[lcl_i] - 6);
                if ((((lcl_by - 1)[lcl_i] - ((-8))) < 0)) {
                    (lcl_ba - 1)[lcl_i] = 0;
                }
            }
        }
    }
    lcl_edx = (lcl_edx + lcl_edir);
    if ((lcl_edx > 24)) {
        lcl_edir = (-1);
    }
    if (((lcl_edx - ((-24))) < 0)) {
        lcl_edir = 1;
    }
    { unsigned char lcl_e = 1; unsigned char L_lim4 = 12;
        for (; lcl_e <= L_lim4; ++lcl_e) {
            if (((unsigned char)(lcl_eal - 1)[lcl_e] != (unsigned char)0)) {
                { int lcl_ex = ((lcl_enx - 1)[lcl_e] + lcl_edx);
                    { int lcl_ey = ((lcl_eny - 1)[lcl_e] + (lcl_scrolly >> 3));
                        if ((lcl_ey > 120)) {
                            lcl_state = 2;
                            gba_music((-1), 1);
                            return;
                        }
                        { unsigned char lcl_i = 1; unsigned char L_lim5 = 8;
                            for (; lcl_i <= L_lim5; ++lcl_i) {
                                if (((unsigned char)(lcl_ba - 1)[lcl_i] != (unsigned char)0)) {
                                    { int lcl_dx = ((lcl_bx - 1)[lcl_i] - lcl_ex);
                                        { int lcl_dy = ((lcl_by - 1)[lcl_i] - lcl_ey);
                                            if ((((((lcl_dx - ((-12))) > 0) && (lcl_dx < 12)) && ((lcl_dy - ((-12))) > 0)) && (lcl_dy < 12))) {
                                                (lcl_eal - 1)[lcl_e] = 0;
                                                (lcl_ba - 1)[lcl_i] = 0;
                                                lcl_score = (lcl_score + 10);
                                                lcl_alive = (lcl_alive - 1);
                                                gba_sfx(0, -1);
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }
    }
    if ((lcl_alive <= 0)) {
        lcl_state = 1;
        gba_music((-1), 1);
    }
}

static void lcl__draw(void)
{
    if ((lcl_state == 1)) {
        gba_print("YOU WIN!", 88, 60, 11);
        gba_print("score", 100, 80, 7);
        gba_print_int(lcl_score, 130, 80, 10);
        gba_print("press A", 96, 100, 12);
        return;
    }
    if ((lcl_state == 2)) {
        gba_print("GAME OVER", 84, 60, 8);
        gba_print("score", 100, 80, 7);
        gba_print_int(lcl_score, 130, 80, 10);
        gba_print("press A", 96, 100, 12);
        return;
    }
    gba_print("score", 4, 4, 7);
    gba_print_int(lcl_score, 44, 4, 10);
    gba_print("lives", 170, 4, 7);
    gba_print_int(lcl_lives, 210, 4, 8);
    gba_spr(0, lcl_px, lcl_py, 2, 2, 0 | (0 << 1));
    { unsigned char lcl_i = 1; unsigned char L_lim6 = 8;
        for (; lcl_i <= L_lim6; ++lcl_i) {
            if (((unsigned char)(lcl_ba - 1)[lcl_i] != (unsigned char)0)) {
                gba_spr8(2, (lcl_bx - 1)[lcl_i], (lcl_by - 1)[lcl_i], 0);
            }
        }
    }
    { unsigned char lcl_e = 1; unsigned char L_lim7 = 12;
        for (; lcl_e <= L_lim7; ++lcl_e) {
            if (((unsigned char)(lcl_eal - 1)[lcl_e] != (unsigned char)0)) {
                gba_spr(1, ((lcl_enx - 1)[lcl_e] + lcl_edx), ((lcl_eny - 1)[lcl_e] + (lcl_scrolly >> 3)), 2, 2, 0 | (0 << 1));
            }
        }
    }
}

int main(void)
{
    gba_init();
    lcl__init();
    for (;;) {
        gba_vsync();
        lcl__update();
        lcl__draw();
        gba_endframe();
    }
    return 0;
}
